...
func TestCreateSubscription(t *testing.T) {
	parentName := "create-subscription"
	...
	cases := map[string]struct {
		...
		"simple": {
			description: "simple",
			options: []scrapligonetconf.Option{
				scrapligonetconf.WithFilter("<stream>snmpevents</stream> "),
			},
		},
	}
	...
	for caseName, c := range cases {
		...
		t.Run(testName, func(t *testing.T) {
			...
			r, err := n.CreateSubscription(ctx, c.options...)
			if err != nil {
				t.Fatal(err)
			}

			// create subscriptions get nuked when the session is closed or killed (or the transport
			// goes away), so no need to tidy up
